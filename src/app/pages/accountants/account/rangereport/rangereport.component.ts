...
export class RangereportComponent implements OnInit, OnDestroy {
  ...
  displayedColumns: string[] = ['no', 'name', 'amount', 'tin_number', 'source_name','source_type_name','category_name','document_type_name','action'];
  ...
  fetchReports() {
    ...
    this.reportService.generateDateReport(start_date, end_date).subscribe({
      next: (response) => {
        this.complaints = response.complains;
        this.dataSource.data = this.complaints; // Fix: Assign data to dataSource
  ...
  //     console.log('SELECT COMPLAIN: ', key);
  ...
  //          const dialogRef = this.dialog.open(FinalmoredataComponent, config);
