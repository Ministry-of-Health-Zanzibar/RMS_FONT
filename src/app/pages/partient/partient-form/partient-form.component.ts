...
export class PartientFormComponent implements OnInit {
  ...
  onSubmit() {
    ...
    Object.keys(basicInfo).forEach(key => {
      ...
      if (val !== null && val !== '') {
        if (key === 'date_of_birth') {
          ...
          // Ensure only numbers are sent. If user typed text that isn't an ID, send nothing or handle error.
          if (!isNaN(Number(val))) {
    ...
    // Map Insurance Info - Correctly handle boolean for FormData
    ...
    this.patientService.addPartient(formData).subscribe({
      ...
      error: (err) => {
        this.loading = false;
        // Logic to display the specific validation error if available
